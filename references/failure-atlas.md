...
| Workbench is right but the published page is still wrong | The deploy bundle was built from a different exported config, or a local draft silently won/lost | Compare the exact exported config hash with the published asset and record draft/published source priority | Do not move any layer. Publish the approved config artifact, then verify source/deploy hashes match |
...
| A position is right in one view but wrong in another | The views disagree on canvas width, units, transform order/origin, font metrics, or responsive scale | Record canvas size, viewport width, computed transform, transform origin, font status, and the layer's raw x/y/scale | Define one coordinate contract and let both consumers use it; do not compensate with per-view magic offsets |
...
| A selected option appears selected on the next question | The handler closes over a mutable current-question index, or the new DOM mounts during the tail of the same mobile tap | Log source question id, target question id, selected answer ids, and transition timing | Bind the callback to its source question id, lock/ignore stale input during transition, and mount the next question with no selected state unless it already has that question's answer |
